...
function newFunction() {
            ...
            prototype: {
                ...
                check: function(b) {
                    ...
                    for (d in f) {
                        ...
                        try {
                            if (c = a.validator.methods[d].call(this, i, b, e.parameters), "dependency-mismatch" === c && 1 === g) { h = !0; continue; }
                            if (h = !1, "pending" === c)
                                return void(this.toHide = this.toHide.not(this.errorsFor(b)));
                            if (!c)
                                return this.formatAndAdd(b, e), !1;
                        } catch (j) { throw this.settings.debug && window.console && console.log("Exception occurred when checking element " + b.id + ", check the '" + e.method + "' method.", j), j; }
                    }
                    if (!h)
    ...
    (function($) {
        ...
        // TODO: Na verdade, permitimos estilos de erro personalizados ou esse é um código herdado?
        var err_style = '';
        try {
        ...
        // Expose extra mc form methods in global var/Expor métodos extras de formulário mc em var global
        window.mc = {
            /**
             *	Open the evil popup/Abra o pop-up do mal
             */
            openPopup: function() {
            ...
            /**
             *	Close the evil popup/Feche o pop-up do mal
             */
            closePopup: function() {
            ...
            /**
             * Descubra se devemos mostrar o pop-up maligno (se eles o fecharam antes, não o mostre).
             */
            evalPopup: function() {
                $('#mc_embed_signup').hide();
            ...
            /**
             * Pegue o URL de inscrição da lista da ação do formulário e faça-o funcionar para um post ajax.
             */
             ...
             */
            /**
             * Classifique as entradas de texto no mesmo grupo de campos que o grupo para fins de validação.
             * Tudo o que isso faz é dizer ao jQueryValidation para criar uma div de erro para o grupo, em vez
             * do que um para cada entrada. O caso de uso principal são os campos de aniversário e data, onde queremos
             * para exibir erros sobre as entradas coletivamente, não individualmente.
             *
             * NOTA: O agrupamento de entradas fornecerá uma div de erro, mas você ainda precisará especificar onde
             * essa div deve ser exibida. Por padrão, é inserido após a primeira entrada com um
             * erro de validação, que pode quebrar um conjunto de entradas. Use a configuração errorPlacement em
             * o validador para controlar o posicionamento de div de erro.
             */
            getGroups: function() {
                var groups = {};
             ...
             */
            /**
             * Chick se um campo for parte de um campo multipartes
             * (por exemplo, um campo de mesclagem de data é composto de entradas individuais para mês, dia e ano)
             * Usado no método onkeyup de validação jQuery para garantir que não avaliamos um campo
             * se um usuário ainda não atingiu a última entrada em um campo de várias partes.
             */
             ...
             */
            /**
             * Verifica se o elemento é a última entrada em seu fieldgroup.
             * Se o campo não for o último de um conjunto de entradas, não queremos validá-lo em determinados eventos (onfocusout, onblur)
             * porque o usuário pode ainda não ter terminado.
             */
             ...
             */
            /**
             * Manipular a mensagem de erro/sucesso após o envio bem-sucedido do formulário.
             * As mensagens de sucesso são anexadas a #mce-success-response
             * As mensagens de erro são exibidas com a entrada inválida quando possível, ou anexadas a #mce-error-response
             */
            mce_success_cb: function(resp) {
                ...
                // No envio bem-sucedido do formulário, exibe uma mensagem de sucesso e redefine o formulário
                if (resp.result == "success") {
                    ...
                    // Se o formulário tiver erros, exiba-os, em linha, se possível, ou anexado a #mce-error-response
                } else {

                    //1 Example errors - Note: You only get one back at a time even if you submit several that are bad. 
                    //2 Error structure - number indicates the index of the merge field that was invalid, then details
                    //3 Object {result: "error", msg: "6 - Please enter the date"} 
                    //4 Object {result: "error", msg: "4 - Please enter a value"} 
                    //5 Object {result: "error", msg: "9 - Please enter a complete address"} 
                    //6 Try to parse the error into a field index and a message.
                    //7 On failure, just put the dump thing into in the msg variable.

                    //1 Erros de exemplo - Nota: Você só recebe um de volta por vez, mesmo se enviar vários que são ruins.
                    //2 Estrutura do erro - número indica o índice do campo de mesclagem que era inválido, então detalha
                    //3 Objeto {resultado: "erro", msg: "6 - Digite a data"}
                    //4 Objeto {resultado: "erro", msg: "4 - Digite um valor"}
                    //5 Objeto {resultado: "erro", msg: "9 - Digite um endereço completo"}
                    //6 Tente analisar o erro em um índice de campo e uma mensagem.
                    //7 Em caso de falha, basta colocar o dump na variável msg.
                    var index = -1;
                    var msg;
                    try {
                    ...
                    try {
                        ...
                        // Se o índice for -1, significa que não temos dados especificamente sobre qual campo era inválido.
                        // Just lump the error message into the generic response div.
                        // Apenas coloque a mensagem de erro na div de resposta genérica.
                        if (index == -1) {
                            $('#mce-' + resp.result + '-response').show();
        ...
        window.mc.mce_validator = $("#mc-embedded-subscribe-form").validate({
            ...
            // Definir erro HTML: <div class="mce_inline_error"></div>
            errorClass: "mce_inline_error",
            errorElement: "div",

            // Validate fields on keyup, focusout and blur/Valide campos no keyup, focusout e blur.
            onkeyup: false,
            ...
            // Agrupar campos faz com que a validação do jQuery exiba um erro para todos os campos do grupo
            // It doesn't have anything to do with how the fields are validated (together or separately),
            // Não tem nada a ver com a forma como os campos são validados (juntos ou separadamente), 
            // it's strictly for visual display of errors
            // é estritamente para exibição visual de erros
            groups: mc.getGroups(),
            // Place a field's inline error HTML just before the div.mc-field-group closing tag 
            // Coloque o HTML de erro embutido de um campo logo antes da tag de fechamento div.mc-field-group
            errorPlacement: function(error, element) {
                element.closest('.mc-field-group').append(error);
            },
            // Submit the form via ajax (see: jQuery Form plugin)
            // Envie o formulário via ajax (veja: plugin jQuery Form)
